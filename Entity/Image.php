...
class Image
{
     ...
     * @var string
     *
     * @ORM\Column(name="sub_folder", type="string", length=32, nullable=true)
     */
    private $sub_folder;
     ...
     * @var string
     *
     * @ORM\Column(name="slug", type="string", length=32, nullable=true)
     */
    private $slug;
    ...
    public function uploadImage( $manual = FALSE, $crop = FALSE, $crop_width = NULL, $crop_height = NULL )
    {
        ...
        $width      = $image_size[ 0 ];
        $height     = $image_size[ 1 ];
//        echo '<br>ORIGINAL SIZE: ' . $width . 'x' . $height;
...
//        echo '<br>ORIG SIZE: ' . $width . 'x' . $height;
    ...
    {
        $folder = $this->getImagesFolder() . DIRECTORY_SEPARATOR . 'tmp';
    ...
    {
        $folder = $this->getMediaFilesFolder() . DIRECTORY_SEPARATOR . 'images';
...
//        $height = $height <= 50 ? 50 : $height;
    ...
    {
        $image = $this->getImagesFolder() . DIRECTORY_SEPARATOR . 'default.jpg';
...
//            echo '<br>PRIVATE: ' . $private . ' ' . ( $has_private ? 'YES' : 'NO' );
//            echo '<br>PUBLIC: ' . $public . ' ' . ( $has_public ? 'YES' : 'NO' );
...
//            echo '<br>' . $size . ' - PRIVATE: ' . $private . ' : ' . ( is_file( $private ) ? '<span style="color:green">YES</span>' : '<span style="color:red">NO</span>' );
//            echo '<br>' . $size . ' - PUBLIC : ' . $public . ' : ' . ( is_file( $public ) ? '<span style="color:green">YES</span>' : '<span style="color:red">NO</span>' );
...
//        $hour  = $date->format( 'h' );
    ...
    public function setSubFolder( $folder = NULL )
    {
        if ( empty( $folder ) ) {
            $folder = 'site';
        }
    ...
    public function remove( $sub_folder = NULL )
    {
        if ( empty( $sub_folder ) ) {
            $sub_folder = strtolower( trim( $this->getType() ) );
     ...
     * @ORM\PreRemove
     */
    public function preremove( $sub_folder = null )
    {
        if( empty( $sub_folder ) ) {
            $sub_folder = $this->getSubFolder();
        }
    ...
    public function getSizeWidth( $size = NULL )
    {
        $sizes = $this->getSizes();
        $size  = strtolower( trim( $size ) );
        $width = $this->getWidth();
        ...
        $size_width = $sizes[ $size ];

        return $size_width;
    }

    public function getSizeHeight( $size = NULL )
    {
        $sizes  = $this->getSizes();
        $size   = strtolower( trim( $size ) );
        $width  = $this->getWidth();
        ...
        return intval( $size_width * $ratio );
    }
